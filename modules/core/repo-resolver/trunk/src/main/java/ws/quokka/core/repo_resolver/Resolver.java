    ...
    private List filterOverrides(String pathId, List overrides) {
        ...
        for (Iterator i = overrides.iterator(); i.hasNext();) {
            ...
            if (override.matches(pathId)) {
                ...
                filtered.add(new RepoOverride(wildcard, override.getGroup(), override.getName(),
                        override.getType(), override.getVersion(), override.getWithVersion(), override.getWithPathSpecs()));
            }
        ...
        for (Iterator i = overrides.iterator(); i.hasNext();) {
            ...
            if (override.matches(dependency.getId())) {
                ...
                overridden.setId(override.getWithVersion() == null ? dependency.getId() :
                        override(dependency.getId(), override.getWithVersion()));
                ...
                for (Iterator j = dependency.getPathSpecs().iterator(); j.hasNext();) {
                    ...
                    if (overiddenPathSpec != null) {
                        ...
                        log.verbose("Overriding path spec for dependency=" + dependency.toShortString()
                                + " from '" + pathSpec.toShortString() + "' to '"
                                + overiddenPathSpec.toShortString() + "'");
...
//        System.out.println("Adding: " + artifact.getId().toShortString() + " declared by " + (declaredBy == null ? "" : declaredBy.toShortString()));
    ...
    private void formatPath(List artifacts, RepoArtifact artifact, boolean onlyConflicted, StringBuffer sb, String indent) {
